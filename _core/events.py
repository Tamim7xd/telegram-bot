...
def format_number(num):
    return f"{num:,}".replace(",", " ").replace(",", ".")  # تنسيق عربي
...
async def dollar_commands(message: Message):
    ...
    # تحذير (للمشرف العادي والأدمن والمشرف الإداري)
        ...
        # تسجيل في سجل التحذيرات (تم في add_warning)
    # معلومات (للمشرفين)
    elif text.startswith("$معلومات") and (is_adm or is_gen_mod or is_adm_mod):
        u = await get_user(target.id)
        if u:
            warnings_list = await get_user_warnings_list(target.id, 1)
            last_warn = warnings_list[0] if warnings_list else None
            last_warn_text = f"آخر تحذير: {last_warn['reason']} (بواسطة {last_warn['admin_id']})" if last_warn else "لا توجد تحذيرات"
            info = f"""📄 *معلومات {u['full_name']}*
💰 الرصيد: {format_number(u['money'])} {CURRENCY_NAME}
⭐ XP: {u['xp']}
📊 المستوى: {u['level']}
...
📝 {last_warn_text}"""
    ...
    # التحذيرات (للمشرفين)
        ...
        warnings = await get_user_warnings_list(target.id, 5)
        if warnings:
...
async def handle_member_commands(message: Message):
    ...
    if text in ["#ملفي", "#حسابي", "#معلوماتي"]:
        ...
        warnings = await get_user_warnings_list(uid, 1)
        last_warn = warnings[0] if warnings else None
        last_warn_text = f"آخر تحذير: {last_warn['reason']}" if last_warn else "لا توجد تحذيرات"
        # الحصول على آخر إجراء (من economy_log)
...
━━━━━━━━━━━━━━━━━━━━━━"""
        ...
        user = await get_user(uid)
        msg = await message.reply(f"🏷️ *لقبك:* {user['title'] or 'لا يوجد'}", parse_mode="Markdown")
        asyncio.create_task(delete_after(msg, 30))
